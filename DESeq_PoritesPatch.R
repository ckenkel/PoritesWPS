...
counts=read.table("AllCountsSym.txt",header=TRUE,row.names=1) #Reading in the table of counts per isogroup by sample - you must open R in the same folder or change the working directory

head(counts) 


length(counts[,1])  #10629 
...
v="/Users/drcarl/Dropbox/AIMSpostdoc/PoritesPatch/AQM_s2"
...
#says that 1N and 7N are outliers, will remove

counts.nobad1=counts[,-c(2,14)] 
# Remove this geno rep and try re-running 
conditions.nobad1=conditions[-c(2,14),]
conditions.nobad1$geno<-factor(conditions.nobad1$geno)
...
pvals.p<-nbinomGLMTest(fit2,fit0) #testing significance of patch
pvals.pg<-nbinomGLMTest(fit3,fit1) #testing significance of patch on top of geno
pvals.pc1<-nbinomGLMTest(fit4,fit1) #testing significance of phys PC1
...
# #look for peak in graph - corresponds to correct theta and best-fit line for which metric to use - pick best theta for all tests
# #geno peak at 0.5, patch peaks at 0.4; pg peaks at ~0.4. pc1 at 0.6 Go with 0.4
...
do<-(cbind(vsd, "fitPatch" = fit3$patchPatch, "adjp.p" = adjp.p,"pval.p" = pvals.p$pvals.p,"adjp.pc1" = adjp.pc1,"pval.pc1" = pvals.pc1$pvals.pc1,"adjp.g" = adjp.g,"pval.g" = pvals.g$pvals.g)) #creating table of all multiple test corrected p-values with variance stabilized count data 


write.csv(do, file="symVSDandPVALS_deseq1_4jun_no_g1Ng7N_gene-est-only_plusPC.csv", quote=F) #writing an output file of vsd plus p-values
...
#SYMS #Top 100 (190 non-overlapping); 500 (=881 non-overlapping); 1000 each (1689), 2000 each, none overlapping (=3144)
...
#par(mfrow=c(1,4))
#FOR SYMS
plot(scores[,1], scores[,2], xlab="PCA 1", ylab="PCA 2",type="n",main="Patch/Geno Top 100")
points(scores[c(1,2,4,6,8,10,11),1],scores[c(1,2,4,6,8,10,11),2],pch=1,col="lightgreen")
points(scores[c(3,5,7,9,12),1],scores[c(3,5,7,9,12),2],pch=19,col="darkgreen")
