...
dat=read.csv("Patchy_Porites_July2019_for_R.csv",stringsAsFactors=TRUE)

summary(dat)

dat$condition=factor(dat$condition,levels=c("healthy","WPS"))
dat$GrossPS_mgO2.cm2.min<-(dat$dayO2_mgO2.cm2.min - dat$nightO2_mgO2.cm2.min)
dat$PR<-(dat$GrossPS_mgO2.cm2.min/abs(dat$nightO2_mgO2.cm2.min))
...
p3<-ggplot(dat,aes(factor(condition),PR,fill=condition))+
	geom_boxplot(outlier.shape=16, notch=FALSE)+
	geom_point(aes(colour=factor(sid)),position=pd,show.legend=FALSE)+
	geom_line(aes(group=factor(sid),colour=factor(sid)),position=pd,show.legend=FALSE)+
	labs(colour="colony")+
...
#########################################################
model3 <- lme(PR~condition,data=dat,random= ~1|sid,method="REML",na.action=na.omit) #change to different traits
...
Fixed effects: PR ~ condition 
                  Value Std.Error DF   t-value p-value
(Intercept)   1.2535127 0.1141144  7 10.984703   0e+00
conditionWPS -0.9119854 0.1613821  7 -5.651093   8e-04
...
ggarrange(pP,pG,ncol=2,labels=c('a','b'),label.args=list(gp=grid::gpar(fint=4,cex=1.2)))


#1st PC explains ~60% of the variation and distinguishes normal from WPS phenotypes, 2nd PC is a lot of genotype, but not totally
